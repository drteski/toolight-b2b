...
const AboutUsData = ({ locale }) => {
	const { data, isLoading } = useGetPayloadData('about-us', true, locale);
	if (isLoading) return <Loading/>;
	return <div className="max-w-inner-wrapper mx-auto my-0">
		...
		<div className="block xs:hidden px-padding pt-padding">
			<Accordion className="flex flex-col gap-padding"
			           defaultValue={data.aboutUs[0].title}
			           type="single"
			           collapsible>
				{data.aboutUs.map((item) => (
					<AccordionItem className="p-padding bg-neutral-100 rounded-xl flex flex-col gap-[calc(var(--spacing-padding)/2)]"
					               key={item.title}
					               value={item.title}>
